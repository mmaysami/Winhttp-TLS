...
{
	Write-Host "`n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
	Write-Host "===> PS2EXE-GUI v0.6.0.1 by Ingo Karstein, reworked and GUI support by Markus Scholtes, `n	further modifications by Stuart Dootson"
}
else
{
...
{
	$psversion = 4
	Write-Host "===> Using PowerShell 4.0 or above." -NoNewline
}

if ($PSVersionTable.PSVersion.Major -eq 3)
{
	$psversion = 3
	Write-Host "===> Using PowerShell 3.0." -NoNewline
}

if ($PSVersionTable.PSVersion.Major -eq 2)
{
	$psversion = 2
	Write-Host "===> Using PowerShell 2.0." -NoNewline
...
$inputBase = Split-Path -Path $inputFile -Leaf

$outputDir = Split-Path -Path $outputFile -Parent
$outputBase = Split-Path -Path $outputFile -Leaf
...
{
	Write-Host "To create a EXE file for PowerShell 2.0 on PowerShell 3.0/4.0 `n	this script now launches PowerShell 2.0..."
	Write-Host ""
...
if ($noConsole)
{
	...
	# Write-Host "===> Local Culture is set to $culture `n"
...
Write-Host "===> Reading input file $inputBase at "
Write-Host "     $inputDir `n"
...
else
{
	...
	if (Test-Path $outputFile)
	{
		Write-Host "<=== Output file $outputBase at "
		Write-Host "	 $outputDir"
		Write-Host "	 written " -NoNewline
	...
	{
		Write-Host "<=== Output file $outputBase at " -ForegroundColor Red
		Write-Host "	 $outputDir" -ForegroundColor Red  -NoNewline
